...
export default function RetailerForm({ onSuccess }) {
	const { addRetailer, isConnected, connectWallet, isLoading, error } = useContract()
	...
	const handleSubmit = async (e) => {
		...
		try {
			const hash = await addRetailer(
				formData.batchId,
				formData.cropName,
				formData.distributorName,
				formData.retailerName,
				formData.shopLocation,
				parseFloat(formData.retailQuantity),
				parseFloat(formData.retailPurchasePrice),
				parseFloat(formData.consumerPrice),
				new Date(formData.expiryDate).getTime() / 1000 // Convert to Unix timestamp
			)
	...
	if (!isConnected) {
		return (
			<div className="bg-white rounded-lg border border-slate-200 p-6">
				<h3 className="text-xl font-semibold text-slate-900 mb-4">Retailer Entry (Blockchain)</h3>
				<div className="text-center py-8">
					<FaExclamationTriangle className="mx-auto text-yellow-500 text-3xl mb-4" />
					<p className="text-slate-600 mb-4">Connect your MetaMask wallet to add retailer info on the blockchain</p>
					<button
			...
			{txHash && (
				<div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-md">
					<div className="flex items-center gap-2 mb-2">
						...
						<p className="text-green-700 font-medium">Retailer added successfully!</p>
					</div>
					<div className="ml-6">
			...
			<form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
				...
				<label className="flex flex-col text-sm">
					...
					<input
						...
						onChange={handleInputChange}
						placeholder="Name of the distributor you bought from"
						className="rounded-md border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
						required
					/>
				</label>
				...
				<label className="flex flex-col text-sm">
					<span className="mb-1 text-slate-700">Retail Quantity (kg) *</span>
					<input
						type="number"
						min="0"
						step="0.01"
